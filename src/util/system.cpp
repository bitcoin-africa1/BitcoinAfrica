...
const char * const BITCOIN_CONF_FILENAME = "bitcoinafrica.conf";
...
#else
    const char* pszModule = "bitcoinafrica";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitcoin-Africa";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitcoin-Africa";
#else
    // Unix
    return pathRet / ".bitcoinafrica";
#endif
#endif
}
